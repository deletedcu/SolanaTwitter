...
pub struct React<'info> {
  // 8 discriminator + 32 user + 32 tweet + 8 timestamp + 1 ReactionChar enum + 1 bump
	#[account(init, payer = user, space = 8 + 32 + 32 + 1 + 1, seeds = [b"reaction", user.key().as_ref(), tweet.key().as_ref()], bump)]
...
pub struct UpdateReaction<'info> {
	pub user: Signer<'info>,
	#[account(mut, seeds = [b"reaction", user.key().as_ref(), reaction.tweet.key().as_ref()], bump = reaction.bump)]
	pub reaction: Account<'info, Reaction>,
}
